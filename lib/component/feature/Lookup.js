...
{	
	...
	{
		var me  = this,
		    source    = me.getRawLookupSelection(),
		    selection = [],
		    model     = me.list.getStore().getProxy().getModel();
		
		if( Ext.isObject( me.mapping ) ) {			
			for( var i = 0, sl = source.length; i < sl; ++i ) {
				selection.push( new model( me.transformRecord( source[ i ] ) ) );
			}
			
		} else {
			for( var i = 0, sl = source.length; i < sl; ++i ) {
				selection.push( new model( source[ i ].data ) );
			}			
		}
