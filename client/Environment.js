Ext.define( 'extAdmin.Environment',
{
	 ...
	 * @param cb
	 */
	init : function( config, cb, scope )
	{
		var me = this;

		me.baseHref = config.baseHref;
...
//		me.moduleManager.init( cb );
	 ...
	 *
	 * @param {extAdmin.Module} [module] Target module
	 * @param {String} [action] Action name
	 * @param {Object} [urlParams=null] Additional request parameters
	 * @returns {String}
	 */
	getActionUrl : function( module, action, urlParams )
	{
		return this.request.buildUrl( module.name, action, urlParams );
	},
	...
	/**
	 * Runs the server action without preoccessing results
	 *
	 * @async
	 * @param {Array} action Action pointer
	 * @param {Object} config Request configuration
	...
	runRawAction : function( action, config )
	{
		...
		me.request.runRawAction( action, config );
	},
